...
static const std::map<std::string, CameraParams> camera_defaults{
    { "assets/cbox.mini", {glm::vec3(0.f, 1.f, 3.5f), glm::vec3(0.f, 1.f, 5.f)} },
...
int main(int argc, char *argv[]) {
    ...
    int ns = 256;
    int nchannels = 3;
    std::string scene_path = "assets/cbox.mini";
    ...
    std::string extension = scene_path.substr(scene_path.size() - 5);

    clock_t t = clock();
    std::cout << "Building BVH... ";
    Scene scene;
    if (extension == ".mini") {
        scene = create_scene(scene_path.c_str());

        // Camera setup (miniScene scenes have no camera data)
        ...
        *cam = Camera();
        ...
        scene = create_mitsuba_scene(scene_path.c_str(), nx, ny);
